...
class Cell {
    ...
    //Los campos no definidos por el constructor se consideran privados y necesitan un getter / setter para acceder desde una clase diferente
    setSize(size) {
        this.radius = size;
    ...
    checkBorder(b) {
        ...
        if (this.position.x < b.minx + r || this.position.x > b.maxx - r) {
            this.boostDirection.x *= 0; // Reflect left-right
        ...
        if (this.position.y < b.miny + r || this.position.y > b.maxy - r) {
            this.boostDirection.y *= 0; // Reflect off of top and bottom, borders
